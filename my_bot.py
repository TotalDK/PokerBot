...
class Bot:
  ...
  def someone_all_in(self, obs):
    for player in obs.player_infos:
      if player.stack == 0 and player.active:
        return True
    return False

  def do_preflop_response(self, obs:Observation):
    if self.someone_all_in(obs):
      if self.rtop.is_hand_in_range(obs.my_hand):
        return max(obs.legal_actions)
      else:
        return 0
    call_odds = obs.get_call_size() / obs.get_pot_size()
    if call_odds < 0.1:
