...
class Bot:
	...
	def do_post_flop_open(self, obs:Observation):
		if self.is_hand_better_than_board(obs):
			return obs.get_fraction_pot_raise(1)
		if self.is_hand_ace_or_better(obs):
			return obs.get_fraction_pot_raise(1)

	def is_hand_better_than_board(self, obs:Observation):
		my_hand_type = obs.get_my_hand_type()
		board_hand_type = obs.get_board_hand_type()
		return my_hand_type > board_hand_type
	...
	def do_preflop_open(self, obs:Observation):
		top_pair_range = self.rtoppair
		open_raise_range = self.r20

		if open_raise_range.is_hand_in_range(obs.my_hand):
			return max(obs.legal_actions) # raise 1 pot
		elif open_raise_range.is_hand_in_range(obs.my_hand):
